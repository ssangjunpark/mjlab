"""Convert .npy motion files to .npz format for training.

This script takes a motion .npy files (with keys: q, fps, mocap, skeleton, joint_names)
...
def run_sim(
  ...
  """Run simulation and save motion data."""
  ...
  while not file_saved:
    ...
    if reset_flag and not file_saved:
      ...
      print(f"Saved motion data to {output_name}")
...
def main(
  ...
  if input_folder.is_file():
    ...
    if output_folder is None:
      output_folder = input_folder.parent / "converted_npz"
  else:
    files = list(input_folder.glob("*.npy"))
    if output_folder is None:
      output_folder = input_folder / "converted_npz"
